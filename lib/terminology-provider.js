...
class TerminologyProvider {
  ...
  getDetails(query) {
    ...
    return this.conceptCollection.find(mongoQuery).skip(offset).limit(limit).toArray()
  }
  ...
  getNarrower(query) {
    ...
    let offset = parseInt(query.offset) || 0
    let uri = query.uri
    return this.conceptCollection.find({ broader: { $elemMatch: { uri: uri } } }).skip(offset).limit(limit).toArray()
  }
