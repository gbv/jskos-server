...
class TerminologyProvider {
  ...
  getConcepts(req, res) {
    ...
    let criteria = {}
    if (query.uri) {
      criteria = { "inScheme.uri": query.uri }
    }
    let cursor = this.conceptCollection.find(criteria)
    return cursor.count().then(total => {
      // Add headers
      ...
      .then(result => {
        ...
        if (concept.broader && concept.broader.length) {
          ...
          if (parentUri == uri) {
            ...
            // TODO: - Remove or deal with differently
  ...
  searchConcept(search, voc) {
    ...
    if (search.length > 1) {
      // Search _keywordsPrefLabel
      queryOr.push({ "_keywordsPrefLabel.de": { $regex: "^" + search.toUpperCase() }})
      queryOr.push({ "_keywordsPrefLabel.en": { $regex: "^" + search.toUpperCase() }})
    }
