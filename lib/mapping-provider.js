...
class MappingProvider {
  ...
  getMappings(query) {
    ...
    return this.collection.find({ $and: [mongoQuery1, mongoQuery2] })
      .limit(limit)
      .toArray()
  }
  ...
  getMappingSchemes(query) {
    ...
    if (query.to) {
      match.push({
        $or: [{
          "to.memberSet.uri": query.from
        }, {
          "to.memberSet.notation": query.from
        },{
          "to.memberChoice.uri": query.from
        }, {
          "to.memberChoice.notation": query.from
        }]
      })
    }
    if (!match.length) {
