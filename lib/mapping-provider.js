...
class MappingProvider {
  ...
  getMappings(req, res) {
    ...
    let count = direction == "both" ? 2 : 1
    query.mode = direction == "both" ? "or" : query.mode
    let fromTo = fromTo => {
      let result
      if (count == 1) {
        result = fromTo == "from" && direction == "backward" ? "to" : "from"
      } else if (count <= 0) {
        return null
      } else {
    ...
    var criteria = ["from", "to"].filter(uri => query[uri]).map(part => {
      ...
      while (count > 0) {
        ...
        if (query[part].startsWith("http")) {
          set[`${side}.memberSet.uri`] = decodeURIComponent(query[part])
          choice[`${side}.memberChoice.uri`] = decodeURIComponent(query[part])
        } else {
          set[`${side}.memberSet.notation`] = decodeURIComponent(query[part])
          choice[`${side}.memberChoice.notation`] = decodeURIComponent(query[part])
        }
        or.push(set, choice)
      }
      return { $or: or }
    ...
    console.log("mongoQuery1", JSON.stringify(mongoQuery1))
    console.log("mongoQuery2", JSON.stringify(mongoQuery2))
    console.log("mongoQuery3", JSON.stringify(mongoQuery3))
    console.log("mongoQuery4", JSON.stringify(mongoQuery4))
    console.log("mongoQuery5", JSON.stringify(mongoQuery5))
