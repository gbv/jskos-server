...
function getFileContent(file) {
  ...
  if (file.startsWith("http")) {
    ...
    promise = new Promise(resolve => {
      try {
        let data = []
        let rs = fs.createReadStream(file, { encoding: "utf8" })
          .pipe(file.endsWith("ndjson") ? ndjson.parse() : JSONStream.parse("*"))
        rs.on("data", object => {
          data.push(object)
          if (data.length % 100 == 0) {
            process.stdout.cursorTo(0)
            process.stdout.write("loaded " + data.length + " objects ")
          }
        })
        rs.on("finish", () => {
          process.stdout.write("\n")
          console.log("Loading from file finished.")
          resolve(data)
        })
        rs.on("error", error => {
          console.error(error)
  ...
  return promise.then(data => {
    ...
    // If data was read from file, return directly.
    if (_.isArray(data)) {
      return data
    }
    let json
...
function importConcepts(concepts) {
  ...
  // 1. Pre-adjustments
  console.log("Performing pre-adjustments for concepts...")
  ...
  for(let object of concepts) {
    ...
    // Check for duplicates, adjust their URIs and notations, and log an error
    if (uris.includes(object.uri)) {
      console.warn("!!!!! Duplicate URI", object.uri)
      let index = 2
      while (uris.includes(`${object.uri}_${index}`)) {
        index += 1
      }
      // Add a note to editorial note
      if (!object.editorialNote) {
        object.editorialNote = {}
      }
      if (!object.editorialNote.en) {
        object.editorialNote.en = []
      }
      object.editorialNote.en.push(`Detected a duplicate URI when importing. Possibly better to use concept with URI ${object.uri}.`)
      // Adjust URI and notation
      object.uri = `${object.uri}_${index}`
      object.notation[0] = `${object.notation[0]}_${index}`
    }
    uris.push(object.uri)
    if (uris.length % 100 == 0) {
      process.stdout.cursorTo(0)
      process.stdout.write("Adjusted " + uris.length + " objects ")
    }
  }
  process.stdout.write("\n")
  ...
  return connect().then(result => {
    ...
    console.log("Writing concepts to database...")
    // 2. Write to database
    promises.push(
      db.collection("concepts").insertMany(concepts, { ordered: false }).then(result => {
        console.log("Concepts written to database, now performing post-adjustments...")
        ids = Object.values(result.insertedIds)
        let done = 0
        // 3. Post-adjustments
  ...
  return connect().then(result => {
    ...
    if (reload) {
      ...
      for (let object of concordances) {
        ...
        promises.push(
          clearConcordance(concordance.uri).then(() => {
...
function importMappings(mappings, { concordance }) {
  // 1. Pre-adjustments
  let count = 0
  console.log("Performing pre-adjustments for mappings...")
  for (let object of mappings) {
    ...
    if (count % 100 == 0) {
      process.stdout.cursorTo(0)
      process.stdout.write("Adjusted " + count + " objects ")
    }
  }
  process.stdout.write("\n")
  ...
  return connect().then(result => {
    ...
    promises.push(
      db.collection("mappings").insertMany(mappings).then(result => {
        console.log("Mappings written to database!")
        ids = Object.values(result.insertedIds)
      })
    )
...
function createIndexes(type) {
  ...
  return connect().then(result => {
    ...
    if (type == "concepts") {
      ...
      indexes.push([{ "_keywordsPrefLabel.de": 1 }, {}])
      indexes.push([{ "_keywordsPrefLabel.en": 1 }, {}])
      indexes.push([
        {
...
/**
 * Drops indexes for a certain type.
 */
function dropIndexes(type) {
  let client, db
  return connect().then(result => {
    client = result.client, db = result.db
    return db.collection(type).dropIndexes().then(() => {
      config.log("Dropped index for collection", type)
    }).catch(() => {
      config.log("Could not drop index for collection", type, "(this can happen e.g. after the collection was dropped first)")
    })
  }).catch(() => null).then(() => {
    client.close()
  })
...
/**
 * Removes all mappings belonging to a certain concordance.
 *
 * @param {*} uri - URI of the concordance
 */
function clearConcordance(uri) {
  let client, db
  return connect().then(result => {
    client = result.client, db = result.db
    return db.collection("mappings").remove({ "partOf.uri": uri }).then(() => {
      config.log("Dropped concordance with URI", uri)
    }).catch(() => {
      config.log("Could not drop concordance with URI", uri)
    })
  }).catch(() => null).then(() => {
    client.close()
  })
